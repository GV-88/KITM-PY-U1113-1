...
def registruotiSpejima(spejimoNr, spejimas, sugeneruotoSkirtumas, failas):
	if(failas):
		if sugeneruotoSkirtumas == 0:
			print(f'{spejimoNr} spėjimu vartotojas atspėjo skaičių', file=failas)
		else:
			print(f'{spejimoNr} spėjimu vartotojas įvedė {spejimas}. Atsakymas - sugeneruotas skaičius {"didesnis" if sugeneruotoSkirtumas > 0 else "mažesnis"}', file=failas)
...
def tikrintiBandyma(spejimoNr, spejimas, atsakymas, nuo, iki, failas):
	...
	if atsakymas < spejimas:
		...
		registruotiSpejima(spejimoNr, spejimas, -1, failas)
		return {'atspejo': False, 'tinkamasBandymas': True}
	registruotiSpejima(spejimoNr, spejimas, 0, failas)
	return {'atspejo': True, 'tinkamasBandymas': True}
...
with open(failas, 'a+', encoding='utf8') as f:
	...
	while zaidziam:
		...
		print(f'Vartotojas įvedė skaičių {n}.', file=f)
		spejimas = 0
		atsakymas = randint(1, n)
		print(f'Sugeneruotas atsitiktinis skaičius {atsakymas}.', file=f)
		...
		while zaidziam:
			...
			tarpinisRezultatas = tikrintiBandyma(kiekSpejimu+1, spejimas, atsakymas, 1, n, f)
			...
			if tarpinisRezultatas['tinkamasBandymas']:
				...
				kiekNetinkamu += 1
				continue
			if not tarpinisRezultatas['atspejo']:
				continue
			...
			while dar == '?':
				...
				match(dar):
					case 't' | 'T':
						zaidziam = True
						print(f'Į užklausą „Ar žaisite dar“ pasirinko „Taip“', file=f)
					case 'n' | 'N':
						zaidziam = False
						print(f'Į užklausą „Ar žaisite dar“ pasirinko „Ne“', file=f)
					case _: dar = '?'
			break # išeiname iš vidinio ciklo su atnaujinta reikšme zaidziam
	print(f'Vartotojas žaidė {kiekZaidimu} kartus(ų)', file=f)
